...
<?php
    ...
    $conn = mysqli_connect($server, $username, $password , $database );
        ...
        $adnum = "IITP/FACREC-CHE/2023/JULY/02";
        $doa = $_POST['doa'];
        $dept = $_POST['dept'];
        $post = $_POST['post'];

         
        $sql = "INSERT INTO applicationdetails (AdvertisementNumber, 	DateOfApplication, APP_NO , 	PostAppliedFor , 	DepartmentSchool ) 
            VALUES (?, ?, ?, ?, ? )
            ON DUPLICATE KEY UPDATE 
            AdvertisementNumber = VALUES(AdvertisementNumber), 
            DateOfApplication = VALUES(DateOfApplication), 
            APP_NO = VALUES(APP_NO), 
          
            PostAppliedFor = VALUES(PostAppliedFor), 
            DepartmentSchool = VALUES(DepartmentSchool)
        ...
        $stmt1->bind_param("ssiss", $adnum,  $doa , $app_no , $post , $dept ) ;
        $stmt1->execute();
        $stmt1->close();
        ...
        $sql = "INSERT INTO personaldetails (APP_NO, FirstName, MiddleName , LastName , 	Nationality, DOB	, 
        Gender, MaritalStatus , Category , IDProof , FathersName , idproofImage , image , 
        CorrAddress, PermanentAddress, Mobile , Email , AltMobile , AltEmail , Landline ) 
            VALUES (?, ?, ?, ?, ?, ?, ?,  ?, ?, ?, ?, ?, ?, ? , ?, ?, ?, ?, ?, ? )
            ON DUPLICATE KEY UPDATE 
            APP_NO = VALUES(APP_NO), 
            ...
            image = VALUES(image),
            CorrAddress = VALUES(CorrAddress),
        ...
        $stmt1 = $conn->prepare($sql);
        $stmt1->bind_param("isssssssssssssssssss", $app_no,  $first_name_welcome , $middle_name_welcome , $last_name_welcome ,
          $nationality, $dob  , $gender , $marital_status , 
          $category , $id_proof , $father_name, $img1 , $img2 , $cadd
          , $padd,$mobile, $email , $email_2 ,$mobile_2 ,  $landline) ;
        $stmt1->execute();
        $stmt1->close();
...
<div class="col-xs-12 col-sm-12 col-md-12 well">
    <form class="form-horizontal" action="welcome.php" method="post" enctype="multipart/form-data">
         ...
         <legend>
          ...
          <div class="row">
            ...
            <div class="col-md-3">
             ...
             <a href='#' class='btn btn-sm btn-info pull-right' onclick="get_username('<?php echo $first_name . " " . $last_name; ?>', 711)" data-target='#passModal' data-toggle='modal'>Change Password</a>
...
<div class="row">
  <div class="col-md-12">
      ...
      <select id="adv_num" name="adv_num" class="form-control input-md" required="">
          ...
          <option selected='selected' name = "adnum" value="IITI/FACREC-CHE/2023/JULY/02">IITI/FACREC-CHE/2023/JULY/02</option>
      </select>
